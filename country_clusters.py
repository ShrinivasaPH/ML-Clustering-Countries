...
def country_clusters():
    ...
    if st.button("Predict Country Type"):
        try:
            ...
            st.header(f"The Country's Cluster: {pred}")
            ...
            st.error(f"Error predicting the cluster: {str(e)}")
...
# Main app navigation
page = st.sidebar.selectbox("Select a Page", ["Country Clusters"])
if page == "Country Clusters":
    country_clusters()
